...
def test_read_matrix_file(capsys):
    ...
    for words, message in words_dict.items():
        ...
        fd, words_file = tempfile.mkstemp()
        open(words_file, 'w').write(content)
        words2 = read_matrix_file(words_file)
        os.close(fd)
        assert words == words2, message + f"\nExpected: " \
                                          f"{words}.\nActual: {words2}."
...
def test_write_output_file(capsys):
    for result in [[('a', 2), ('b', 3)], [('a', 2)], []]:
        ...
        write_output_file(result, filename)
...
def _find_words_in_matrix_wide(suffix):
    ...
    words = read_wordlist_file(os.path.join(EX_DIR, f'words{suffix}.txt'))
    matrix = read_matrix_file(os.path.join(EX_DIR, f'matrix{suffix}.txt'))
    for direction in set(directions):
        pairs = find_words_in_matrix(words, matrix, direction)
        actual = sorted(pairs)
    ...
    for direction1 in directions:
        for direction2 in directions:
            for direction3 in directions:
                ...
                direction_ok = ''.join(set(list(direction_str)))
                pairs = find_words_in_matrix(words, matrix, direction_ok)
                actual = sorted(pairs)
                expected_dict = {}
...
def test_main_wide(capsys):
    ...
    for direction in directions:
        ret = subprocess.check_call(
            python_path + f' -m wordsearch '
                          f'{os.path.join(EX_DIR, "words.txt")} '
                          f'{os.path.join(EX_DIR, "matrix.txt")} '
                          f'{OUTPUT_FILE} ' + direction)
